...
/**
 * memo组件渲染的时候并不会去渲染memo(MemoFunctionCounter)组件这个标签本身
 * 而是执行MemoFunctionCounter编译后的虚拟DOM的type函数
 * 把type代表的函数式组件渲染到对应的位置
 * 总结一下：MemoFunctionCounter只是个占位的 MemoFunctionCounter的type代表的函数式
 * 才是真正要渲染的MemoFunctionCounter位置上的真实DOM
 * mountMemoComponent()组件返回的是函数式组件代表的真实DOM
 * @param {*} vdom 
 * @returns 
 */
function mountMemoComponent(vdom) {
  // 解构出type：函数式组件的引用 和props
  let {type, props} = vdom;
  // 执行函数式组件的函数 经过babel编译成 虚拟DOM
  let renderVdom = type.type(props);
  // 在虚拟DOM上定义一个oldRenderVdom保存的是上一次的虚拟DOM
  vdom.oldRenderVdom = renderVdom;
  return createDOM(renderVdom);
}
