...
function runSaga(env, saga, callback) {
    ...
    function next(val, isError) {
        let result;
        if (isError) {
            result = it.throw(val); // 立即停止saga 表示出错了 实现callback时加的
        } else if (val === TASK_CANCEL) {
            result = it.return(val); // 立即停止saga
        }
        else {
            result = it.next(val);
        }
            ...
            if (typeof effect[Symbol.iterator] === 'function') {
                ...
                switch (effect.type) {
                    ...
                    case effectTypes.FORK:
                        let forkTask = runSaga(env, effect.saga); // 递归重新开启一个迭代器执行effect这个saga 有点类似开启一个子进程
                        next(forkTask); // 此saga继续执行 不会被这个类似子进程的saga卡住
                        break;
